...
## Table of contents
- [Certified Operator Index: Container Image Signature Verification Failures](#certified-operator-index-container-image-signature-verification-failures)
...
```
GET /v2/openshift4/ose-csi-external-attacher-rhel9/manifests/sha256-8c7a20b3...22.sig HTTP/1.1  →  200  (24349 bytes)
PUT /v2/openshift4/ose-csi-external-attacher-rhel9/manifests/sha256-8c7a20b3...22.sig HTTP/1.1  →  201
```
...
```
GET /v2/netapp/trident-autosupport/manifests/sha256-566667b5...97.sig HTTP/1.1  →  404  (169 bytes)
```
...
PrepareRegistrydCustomDir()                    // registriesd.go — writes use-sigstore-attachments: true
  └→ sourceCtx.RegistriesDirPath = ...        // mirror.go:114 — tells library where to find the config
            ...
            └→ GET .../sha256-DIGEST.sig      // HTTP 404 from registry.connect.redhat.com
                 └→ "reading signatures: reading manifest ... name unknown: Image not found"
                      └→ err returned to batch worker
                           └→ errArray = append(errArray, *err)      // concurrent_chan_worker.go:188
                                └→ shouldSkipImage() matches bundle  // concurrent_chan_worker.go:379-389
                                     └→ bundle skipped
```
...
The `--remove-signatures` flag (`internal/pkg/cli/executor.go:276`) prevents setting
`RegistriesDirPath` on the SystemContext AND sets `RemoveSignatures: true` on the copy
options. This avoids the signature read error entirely.
